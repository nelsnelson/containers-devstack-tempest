...
puppet apply --modulepath=/root/system-config/modules:/etc/puppet/modules -e "class { openstack_project::single_use_slave: install_users => false, ssh_key => \"$( cat /root/.ssh/id_rsa.pub | awk '{print $2}' )\" }"

wait ${!}
return_code=$?
echo "${return_code}" > /tmp/openstack-infra-finished
